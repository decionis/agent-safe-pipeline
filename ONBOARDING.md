...
The adopter's journey through this repository, packaged once and walked five times. The shape is
...
| 7. Outcome      | `COMPLETED`, `BLOCKED`, `FAILED_BEFORE_DISPATCH`, `DEFINITELY_NOT_EXECUTED` or `UNKNOWN_AFTER_DISPATCH`; an unknown outcome is reconciled, never retried on its own | [docs/execution-outcomes.md](./docs/execution-outcomes.md)                                                   |
...
| Banking              | `packages/agentsafe` adapters (the executor-side BEAP subset), with `profiles/beap/v1.0` mirrored here as the profile it was written against and the frozen 0.1 draft beside it    | [banking.decionis.com](https://banking.decionis.com) — BEAP v0.1, Draft / Design Partner Review                                       | A banking action was bound to its own digests, executed once, and its effect read back and compared                      | Anything about a bank's production path; no conformance level is claimed, and the profile's own runtime is not here    |
| Proof of human       | `examples/local-escalation` (both Presence integration modes against loopback doubles, ceremony simulated), then `examples/presence-live-approval`                                 | [presence.decionis.com](https://presence.decionis.com) — the adaptive human verification layer; production enforcement sales-assisted | An ESCALATE was resolved by a verified person's signed Presence Record and re-evaluated before any grant                 | That a real ceremony ran: the loopback double simulates it, and the live example needs the tenant's own credentials    |
...
| Infrastructure       | `examples/infra-scale-demo` (the Compromised Principal Test: a valid identity proposing a `deployment.scale` nobody authorised), then `deploy/` for the same boundary in a cluster | [decionis.com](https://decionis.com)                                                                                                  | An agent whose identity, credential and API were all valid could not scale beyond, outside, or after what was authorised | That the cluster's CNI enforces the kit's NetworkPolicies; see `deploy/README.md` and THREAT-MODEL.md "Accepted risks" |
