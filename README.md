...
- [`govern/`](./govern) — Govern, the gate for workflows: one Go binary for GitHub Actions, GitLab CI, Jenkins and any other runner that captures a step as an execution intent, asks Decionis through `enforce-and-bind`, runs the command only on a claimed grant, and finalizes the outcome into the Decision Dossier; the `decionis/govern` action's source since 2026-09-21 (v2; v1 was the node20 action on the evaluate-decision API); [`docs/govern.md`](./docs/govern.md) is the page.
