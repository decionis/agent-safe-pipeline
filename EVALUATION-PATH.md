...
deployable process on its own version, `@decionis/commerce`, a local MCP server over the CommerceGate
...
| The host posture the process can observe (non-root, read-only root and working directory, no service-account token, a clean environment, private secret files, Node's permission model), secret handling and rotation                                                                                                                     | `@decionis/agentsafe`, verified at start and while running, refusing to run without it                      | [packages/agentsafe/README.md](./packages/agentsafe/README.md) "Host posture"                                                                                               |
| Outbound connections, the listener, and the evidence stream (egress sealed to the configured origins with declared CA bundles and SPKI pins; a TLS 1.3 listener with an optional client CA; hash-chained evidence and security streams verifiable offline)                                                                                | `@decionis/agentsafe`, enforced in the process and checked by its tests, mutation gate and proof            | [packages/agentsafe/README.md](./packages/agentsafe/README.md) "Egress", "Evidence and metrics"; [docs/executor-evidence.md](./docs/executor-evidence.md)                   |
| Who may call (principals with roles, scopes and their own credentials: a bearer digest, a client certificate, or a workload token verified against the cluster JWKS), what each may propose, and how the executor proves itself downstream (a static header, a `private_key_jwt` client-credentials token, or an RFC 9421 signed request) | `@decionis/agentsafe`, enforced at the door and before capture, checked by its tests and mutation gate      | [packages/agentsafe/README.md](./packages/agentsafe/README.md) "Principals", "Downstream credentials"                                                                       |
| Whether anything runs at all, and whether a lost outcome can be recovered (a durable attempt journal written before dispatch, read-only startup reconciliation, an operator and automatic halt, host ceilings, and a clock guard against the authority)                                                                                   | `@decionis/agentsafe`, enforced in the process and proved by its tests, mutation gate and offline drills    | [packages/agentsafe/README.md](./packages/agentsafe/README.md) "The attempt journal", "The halt", "Host ceilings", "Clocks"                                                 |
| Incident response capabilities: an evidence bundle an operator can take from a running process and verify offline, the playbooks for six incident classes, alerting rules as data, and offline drills that prove each one                                                                                                                 | `@decionis/agentsafe`, with the playbooks written to be adopted rather than run as-is                       | [docs/incident-response.md](./docs/incident-response.md), [deploy/incident/](./deploy/incident), [deploy/alerts/TrustedExecutor.yaml](./deploy/alerts/TrustedExecutor.yaml) |
| Executor isolation itself (the container runtime, Pod Security Admission, the enforcement of a network policy by a CNI), node and kernel isolation, secret provisioning, a KMS or HSM, who on-call actually is                                                                                                                            | Your cluster; declared and conformance-tested in `deploy/`, and not verifiable from inside the process      | [deploy/README.md](./deploy/README.md), [THREAT-MODEL.md](./THREAT-MODEL.md) "Accepted risks", [docs/trust-boundary.md](./docs/trust-boundary.md)                           |
...
| The self-checking demos (`examples/golden-adversarial-demo`, `examples/whisper-boundary-demo`, `examples/crm-outreach-demo`, `examples/infra-scale-demo`), run by `pnpm examples:prove` in the CI verify job | Against synthetic policy, a fixture authority and in-process doubles, every legitimate leg executed once and every attack leg did not execute; each demo exits non-zero on any failed leg, and CI runs all four on every pull request; the infrastructure demo is the Compromised Principal Test, a valid identity proposing an unauthorized action | That a hosted integration exists; that any real provider, model, or deployment behaves this way                                    |
| The trusted executor proof (`examples/trusted-executor` over `@decionis/agentsafe`), run by `pnpm examples:prove`                                                                                            | Over real HTTP against the loopback doubles: shadow never executes, one `ALLOW` is one dispatch, refusals at the door hold, a lost provider response is reconciled without a second send, and no credential, token or key reaches a response or an audit line                                                                                       | That the host's isolation, egress denial or credential scoping is in place; anything about a real provider, cluster, or deployment |
...
2. `pnpm examples:prove` — the four offline self-checking demos (golden adversarial, whisper
...
6. `pnpm --filter @decionis/agent-safe-example-trusted-executor demo` — the executor as a process,
   proved over real HTTP against the loopback doubles; then read [deploy/Runbook.md](./deploy/Runbook.md)
   for what changes between shadow and enforcement once it is deployed.
