...
| The trusted executor proof (`examples/trusted-executor`), run by `pnpm examples:prove`                                                                                          | Over real HTTP against the loopback doubles: shadow never executes, one `ALLOW` is one dispatch, refusals at the door hold, a lost provider response is reconciled without a second send, and no credential, token or key reaches a response or an audit line | That the host's isolation, egress denial or credential scoping is in place; anything about a real provider, cluster, or deployment |
