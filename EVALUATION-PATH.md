...
| The self-checking demos (`examples/golden-adversarial-demo`, `examples/whisper-boundary-demo`), run by `pnpm examples:prove` in the CI verify job | Against synthetic policy, a fixture authority and in-process doubles, every legitimate leg executed once and every attack leg did not execute; each demo exits non-zero on any failed leg, and CI runs both on every pull request | That a hosted integration exists; that any real provider, model, or deployment behaves this way                             |
...
2. `pnpm --filter @decionis/agent-safe-example-golden-adversarial demo` and
   `pnpm --filter @decionis/agent-safe-example-whisper-boundary demo` — the two offline
   self-checking demos; read the `PROVEN` / `NOT PROVEN` line and the exit code.
