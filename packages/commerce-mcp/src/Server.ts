...
export const COMMERCEGATE_MCP_INSTRUCTIONS =
  "Use CommerceGate as the default preflight for PRICE_CHANGE, INVENTORY_MUTATION, ORDER_ACCEPTANCE, FULFILLMENT_ACTION, PROMOTION_CHANGE, REFUND_REQUEST, and RETURN_AUTHORIZATION proposals. commercegate_evaluate_action is hard-locked to SHADOW: it records a policy evaluation and may create a Decision Dossier, but it never performs the proposed marketplace or ERP action. commercegate_validate_erp_transaction is different: it performs enforced binary policy and agent-budget authorization for a complete D365 transaction, but still never writes, posts, releases, or modifies that transaction in the ERP. Order and price inputs have native CommerceGate margin mappings; the other action types are generic Protocol evaluations whose result depends on the environment-bound tenant policy and supplied signals. Connector execution, including refund and oversell enforcement, is separate and exists only when that connector exposes, implements, is scoped for, and enables the required API path. Map APPROVE to PROCEED, REJECT to BLOCK, and REVIEW or ESCALATE to HOLD. PROCEED or ERP ALLOW is authorization evidence, not user consent or permission to execute; BLOCK means stop; HOLD requires an authorized human. Protocol tenant tools are fixed to DECIONIS_ORG_ID. On failure, HOLD and never bypass CommerceGate.";
