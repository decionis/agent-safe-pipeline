...
// Found by walking up, so the mutation sandbox nested in the package finds it too.
const vectorPath = (name: string): string => repositoryPath("conformance", "vectors", name);

async function corpus(): Promise<Record<string, string>> {
  const principal = await readFile(vectorPath("compromised-principal.json"), "utf8");
  const zero = await readFile(vectorPath("negative-zero.json"), "utf8");
