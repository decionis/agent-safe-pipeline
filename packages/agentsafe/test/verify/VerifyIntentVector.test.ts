...
// The corpus is a repository artefact, found by walking up: the same test runs
// from the package and from a mutation sandbox nested inside it.
const VECTORS_DIR = repositoryPath("conformance", "vectors");
const PINNED = repositoryPath("conformance", "agent-safe-intent-v1.json");

type Json = Record<string, unknown>;

async function vector(name: string): Promise<Json> {
  return JSON.parse(await readFile(join(VECTORS_DIR, name), "utf8")) as Json;
}
