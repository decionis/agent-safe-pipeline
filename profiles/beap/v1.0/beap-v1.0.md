# Decionis Banking Execution Authority Profile (BEAP) v1.0

| Field                       | Value                                 |
| --------------------------- | ------------------------------------- |
| Version                     | 1.0                                   |
| Status                      | Published                             |
| Profile identifier          | `decionis.beap/v1.0`                  |
| Protocol                    | Decionis Execution Authority Protocol |
| Intended canonical location | `banking.decionis.com/spec/v1.0`      |
...
This is version 1.0 of the profile. Design-partner review of the 0.1 draft has concluded; 0.1 is frozen and archived, and this version is the first to carry the compatibility promise of section 8: a later 1.x version may add optional fields and registry entries, and may not change the canonicalization rule or the meaning of an existing field, verdict, or outcome token.

It is the profile that Decionis publishes and implements. It is not a standard approved by any standards body, and this document makes no conformance claim on anyone's behalf: a claim is made by an implementation under section 24.4, naming the version it targets and the level it satisfies. Section 24.5 names the implementations Decionis maintains as references, by the layer each is a reference for.

Feedback remains welcome from banks, fintechs, core-banking implementers, payment platforms, lending platforms, and risk and compliance teams, and is taken into the next version. A published version is never edited; corrections appear in a subsequent version, and the change history is kept in the change log (Appendix D).
...
Every BEAP artifact (BankingAction, Batch Manifest, AuthoritySet, Effect Evidence, grant claims, dossier projection) MUST carry the field `profile` with the value `decionis.beap/v1.0`. [BEAP-L1-ACT-04]
...
| ------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `profile`           | yes      | `decionis.beap/v1.0`                                                                                                                  |
...
The registered domains for version 1.0 are listed in the execution-domain registry (section 26):
...
### 19.1 Claim attestation

A claim token proves a claim to the execution authority, and to no one else: it is opaque by design, and a downstream banking system that receives an instruction from a Trusted Executor has, from the token, no way to tell whether the authority ever claimed the grant behind it. The claim attestation closes that gap. It is the execution authority's own signed statement, returned with a successful claim, that this grant was claimed for this Intent Digest and is live until the lease expires, verifiable against the authority's published keys by anyone who holds the request.

```text
claim ──► claim token (to the executor, for finalization)
      └─► claim attestation (forwarded downstream, verifiable by the banking system)
```

Where the execution authority returns a claim attestation with a successful claim, the Trusted Executor MUST forward it with the downstream request, unaltered, and MUST cover it, together with the grant identifier and the decision identifier, in any signature it produces over that request. [BEAP-L3-CLM-10]

A claim attestation MUST identify the grant, the decision, the Intent Digest, the digest the authority bound over the canonical action parameters together with the canonicalization profile it used, and the lease expiry, and MUST be signed with a key the authority publishes for offline verification. It MUST NOT carry the claim token or any action parameter; where it refers to either, it carries a digest. [BEAP-L3-CLM-11]

A downstream banking system MAY require a claim attestation before it effects a consequential operation. Where it does, it MUST verify the attestation's signature against the authority's published keys, MUST refuse an attestation whose lease has expired, MUST refuse a request whose grant identifier, decision identifier, Intent Digest, or canonical payload digest differs from the attestation's, and SHOULD refuse a grant identifier it has already effected within the lease. [BEAP-L3-CLM-12]

An attestation attests that a claim was live when it was signed. It does not extend the lease, revive a consumed grant, or substitute for finalization (section 19); a downstream that needs "still live" at commit reads the lease, and one that needs "not already used" keeps the grant identifier for the length of it.
...
Where the Trusted Executor proves itself to a downstream banking system with a signature over the request rather than a bearer credential, that signature SHOULD cover the grant identifier, the decision identifier, and the claim attestation of section 19.1, so that the downstream can refuse an instruction the execution authority never claimed and not only one the executor never signed. [BEAP-L3-CRD-05] A bearer credential proves only that its holder sent the request; a captured bearer is usable by whoever holds it, and a downstream that accepts one is relying on the network to keep the proposing side away, which section 21 exists to make unnecessary.
...
### 24.5 Reference implementations

Decionis maintains one reference implementation per layer of the profile. Each is a reference for the layer named and for nothing beyond it; being named here is not a conformance claim, and each states in its own documentation the version it targets and the requirements it satisfies, under section 24.4.

| Layer                                                                                                                                                                                                                                                                                                                    | Reference implementation                                                                          | Where                                                     |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| Banking semantics: the canonical BankingAction and its canonicalization, the Intent Digest, execution domains, policy resolution, verdicts, AuthoritySets, batch binding, expected-effect projection and effect comparison                                                                                               | `@decionis/banking-contracts` and `@decionis/banking-core`                                        | This repository                                           |
| The authority-side harness for development and conformance runs: a reference execution authority over the reference policy evaluator, single-use grants, claim-time revalidation, a decision chain; refuses to run in production                                                                                         | `@decionis/banking-runtime`                                                                       | This repository                                           |
| The Trusted Executor as a deployable process: claim-before-commit, the adapter contract, credential isolation, the claim attestation, effect evidence and reconciliation, and the host controls around them (verified posture, caller principals, sealed egress, a durable attempt journal, halt, hash-chained evidence) | `@decionis/agentsafe`, with its banking adapter family                                            | The Agent-Safe Pipeline repository (`packages/agentsafe`) |
| The execution authority in production: policy, grants, dossiers, claims and their attestations, the L1 and L2 requirements as operated                                                                                                                                                                                   | The Decionis service; its contract is the Decionis OpenAPI, to which Appendix B maps this profile | Decionis                                                  |

The two runtimes are complementary, not alternatives. `@decionis/banking-runtime` is the thin reference over the published pipeline that speaks the profile's vocabulary and stands in for the authority in examples and conformance runs; `@decionis/agentsafe` is the process an institution deploys at the execution boundary. An adapter written to the contract of section 20 runs under either.
...
| The claim attestation (section 19.1)                      | `claim_attestation` on the protocol's claim response: a compact EdDSA JWS over `ClaimAttestationClaims`, verifiable against `/.well-known/decionis-execution-grant-jwks.json` |
...
### B.6 The claim attestation on the wire

The protocol's claim response carries `claim_attestation` beside `claim_token` on a live claim, and nothing on a refused one. Its payload names the grant (`sub`, the grant's `jti`), `org_id`, `dossier_id`, `decision_id`, the grant's `binding` (`intent_hash`, `execution_payload_digest` with `execution_payload_canonicalization_profile`, `expected_effect_digest`, `execution_nonce`, `execution_correlation_id`), `claim_token_digest`, `claim_validated_at`, and a lifetime whose `exp` is the claim lease; the protected header carries `alg: EdDSA`, the signing `kid`, and `typ: decionis-claim-attestation+jwt`. The keys are the execution-grant keys the authority already publishes, so a verifier of grants is a verifier of attestations.

The reference executor forwards it as the request header `x-agent-safe-claim-attestation`, beside `x-agent-safe-grant-id` and `x-agent-safe-decision-id`, and its `SIGNED_REQUEST` credential covers all three in the RFC 9421 signature it produces, so a downstream can require the signature to cover them. The verification procedure a downstream runs is written out in that executor's documentation and exercised by its offline proof.
...
Version 1.0 (published, 2026-09-15): the first published version, carrying the compatibility promise of section 8. Design-partner review of the 0.1 draft concluded; 0.1 is frozen and archived under the Agent-Safe Pipeline repository's concept DOI. The profile identifier is `decionis.beap/v1.0`, and every artifact carries it. Added the claim attestation (section 19.1, BEAP-L3-CLM-10 to CLM-12): the execution authority's signed statement, returned with a successful claim and verifiable against its published keys, that a grant was claimed for an Intent Digest, so that a downstream banking system can refuse an instruction the authority never claimed and not only one the executor never signed; and the corresponding recommendation that a request signature cover the grant, the decision, and the attestation (BEAP-L3-CRD-05). Named the reference implementations by layer (section 24.5) and mapped the attestation to the protocol (Appendix B.5, B.6). No canonicalization rule, field meaning, verdict, or outcome token changed from 0.1.
