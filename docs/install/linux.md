...
profile, never installs a service, and touches nothing else. `AGENTSAFE_VERSION` pins a version;
`AGENTSAFE_INSTALL_DIR` chooses the directory.
...
sudo rpm -i agentsafe-<version>.x86_64.rpm      # RHEL, Fedora, SUSE
```
...
sudo cp /etc/agentsafe/environment.example /etc/agentsafe/environment
